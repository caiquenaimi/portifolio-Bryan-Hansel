...
import { Sparkles, Zap, GitBranch, PenTool, Layout, Layers, Users, MousePointer2 } from "lucide-react";
...
/* ===== Ferramentas & Plataformas (Recuperadas e Estilizadas) ===== */
const softwares = [
    ...
    <section id="skills" className="py-24 md:py-32 px-6 md:px-12 lg:px-24 bg-secondary">
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          {/* Ferramentas & Plataformas (Versão Inovadora integrada) */}
          <div className="space-y-8">
            <h3 className="text-sm tracking-widest uppercase text-muted-foreground">Ecossistema de Ferramentas</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {softwares.map((sw, index) => (
                <motion.div
                  key={index}
                  whileHover={{ borderColor: "var(--accent)", y: -5 }}
                  className="group flex flex-col gap-6 p-8 bg-background/50 border border-border backdrop-blur-sm transition-all relative overflow-hidden rounded-2xl"
                >
                  <div className="absolute -top-6 -right-6 w-16 h-16 bg-accent/5 rounded-full group-hover:bg-accent/15 transition-colors" />
                  
                  <div className="flex justify-between items-start relative z-10">
                    <sw.icon className="w-8 h-8 text-accent/70 group-hover:text-accent transition-colors" />
                    <span className="text-[10px] text-zinc-500 uppercase tracking-[0.2em] font-medium">{sw.category}</span>
                  </div>
                  
                  <div className="space-y-2 relative z-10">
                    <span className="font-[family-name:var(--font-heading)] text-xl md:text-2xl font-bold block">
                      {sw.name}
                    </span>
